...
int main(int argc, char *argv[]) {
  //Usage
  if (argc < 3) {
    cout << "Usage: LU foo.txt <panel width>" << endl;
    exit(1);
  }

  char* matrixFile = argv[1];
  int panelWidth=atoi(argv[2]);
  Matrix A(matrixFile);
  A.display();
  LUFactorization lu(A);

  cout << "Right Looking Block LU" << endl;
  LUFactorization result = lu.rightLookingBlockLU(panelWidth);
  result.display();
